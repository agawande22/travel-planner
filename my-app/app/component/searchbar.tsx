            ...
            <Box  sx={{ flex:1.5, height: 80, bgcolor: color, border: '2px solid black', borderRadius: 10, minWidth: '800px'}}>
              <Grid2 container spacing={1} >
                <Grid2 size={{ xs: 6, md: 2 }} >
                    <PlaceAutocomplete onPlaceSelect={setFromPlace} />
                </Grid2>
                <Grid2 size={{ xs: 6, md: 2 }} >
                  {/* <TextField variant="standard" placeholder="City, State, Country" value={localLocation} onChange={(e) => {setLocalLocation(e.target.value);}}
                    ...
                    <PlaceAutocomplete onPlaceSelect={setToPlace} />
                </Grid2>
                <Box sx={{color: contrast, alignSelf:'center', width: 0, height: 50, border: `1px solid ${contrast}`, mx: 2 }}></Box>                    
                <Grid2 size={{ xs: 6, md: 2 }} >
                  <DatePicker label="Check-in" value={checkIn} onChange={(newValue: Date | null) => {setLocalCheckIn(newValue);}}
                    slots={{ textField: (props) => <TextField {...props} variant="standard" />, }}
                    slotProps={{ textField: { InputProps: { disableUnderline: true }, sx: { flex: 1, my: 2 }, required: true }, }}  />
                </Grid2>
                <Box sx={{color: contrast, alignSelf:'center', width: 0, height: 50, border: `1px solid ${contrast}`, mx: 2 }}></Box>
                <Grid2 size={{ xs: 6, md: 2 }} >
                  <DatePicker label="Check-out" value={checkOut} onChange={(newValue: Date | null) => {setLocalCheckOut(newValue);}}
                      slots={{ textField: (props) => <TextField {...props} variant="standard" />, }}
                      slotProps={{ textField: { InputProps: { disableUnderline: true }, sx: { flex: 1, my: 2 }, required: true }, }} />
                </Grid2>
                <Box sx={{color: contrast, alignSelf:'center', width: 0, height: 50, border: `1px solid ${contrast}`, mx: 2 }}></Box>
                <Grid2 size={{ xs: 6, md: 2 }} >
                  <Box sx={{display: 'flex', alignItems: 'center', gap: 2, flex: 1, m: 3, justifyContent: 'center', }}>
                    ...
                    <Typography sx={{ width: '30px', textAlign: 'center', fontSize: '1.2rem', }} > {guests} </Typography>
